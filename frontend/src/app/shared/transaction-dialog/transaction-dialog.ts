...
export class TransactionDialog {
  ...
  readonly form: TransactionForm = this.fb.group({
    ...
    categoryId: this.fb.control(
      this.firstCategoryId(this.data.mode === 'income' ? 'income' : 'expense'),
      {
        validators: Validators.required,
      },
    ),
  ...
  readonly hasNoCategories = computed(() => this.availableCategories().length === 0);
  readonly hasNoLoans = computed(() => this.linksToLoan() && this.data.loans.length === 0);
  readonly isMissingRequiredList = computed(() => this.hasNoCategories() || this.hasNoLoans());
